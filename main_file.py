...
import datetime
import os
from forms.user import LoginForm, AddModerator, AddTeacher, \
...
@app.route('/moderinfo', methods=['GET', 'POST'])
@login_required
def moder_info():
    '''Вывод информации по использованию сайта для модератора'''
    form = Back()
    return render_template('info.html', title='Просмотр расписания',
                           form=form, value=['Сверху в левой части экрана мы видим 2 надписи',
                                            '1. Данные: Нажав на неё выпадет список с возможностью Добавить или Изменить/Посмотреть уже добавленные данные',
                                            'В этом списке будет выбор из Модератора, Учителя, Ученика, Класса или предмета',
                                            'И самое главное!!!! Заполнение идёт следующим образом Добавление предмета/предметов -> Добавление учителя/учителей -> Добавление класса/классов -> Добавление ученика/учеников',
                                            'Есть возможность редактирования данных учителя, ученика, а также можно удалять данные о предмете/классе/учителе/ученике из базы данных при помощи соответствующей кнопки в разделе Данные -> Изменение/Просмотр',
                                            ...
                                            '-- 1.Для Добавления или Изменения расписания нужно нажать Расписание -> Добавить/Изменить, после чего выбрать класс и день недели, на который пользователь хочет добавить или изменить расписание',
                                            'Добавление и Изменение делается одинаково: просто выбираете предмметы по порядку или можно сделать пропуск, то есть отсутсвие урока, выбрав вместо названия предмета прочерк и после всего нажимаете на кнопку Добавить. Всё, новое расписание добавлено!',
                                            '-- 2. Можно просто посмотреть расписание любого класса, проследовав Расписание -> Просмотр, после чего выбрать класс и подтвердить выбор класса нажатием на кнопку и расписание нужного класса окажется прямо перед вами, при нажатии на день недели, расписание раскроется, при повторном нажати на день недели, расписание спрячется'])


@app.route('/teacherinfo', methods=['GET', 'POST'])
@login_required
def teacher_info():
    '''Вывод информации по использованию сайта для учителя'''
    form = Back()
    return render_template('info.html', title='Просмотр расписания',
                           form=form, value=['Сверху в левой части экрана мы видим 3 надписи',
                                             '1. Уроки: Нажав на эту надпись учитель перейдет в новое окно, где сможет выставить оценку ученику, а именно:',
                                             '-- 1.Выбрать класс, подтвердить выбор, нажав кнопку',
                                             '-- 2.Выбрать предмет, подтвердить выбор, нажав кнопку',
                                             '-- 3.Выбрать ученика и оценку и также подтвердить, нажав на кнопку',
                                             '-- 4.Поздравляем! Оценка выставлена.',
...
@app.route('/pupilinfo', methods=['GET', 'POST'])
@login_required
def pupil_info():
    '''Вывод информации по использованию сайта для ученика'''
    form = Back()
    return render_template('info.html', title='Просмотр расписания',
                           form=form, value=['Сверху в левой части экрана мы видим 3 надписи',
